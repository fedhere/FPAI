...
def extract_indices(dest_state_intersection, STATE):
    ...
    features_state = dest_state_intersection.copy()
    ...
    dest_state_intersection_wfeatures = dest_state_intersection.join(dfall)
    ...
    dest_state_intersection_wfeatures[['area', 'length', 'inv_isoper', 'ave_r',
       'ave_g', 'ave_b', 'std_r', 'std_g', 'std_b',
       'MSAVI', 'NDBI', 'NDVI', 'NDWI', 'MSAVI_c', 'NDBI_c', 'NDVI_c',
       'NDWI_c']] = dest_state_intersection_wfeatures[['area', 'length', 'inv_isoper', 'ave_r',
       'ave_g', 'ave_b', 'std_r', 'std_g', 'std_b',
       'MSAVI', 'NDBI', 'NDVI', 'NDWI', 'MSAVI_c', 'NDBI_c', 'NDVI_c',
       'NDWI_c']].astype('float32')
...
def check_preds_within_state(DATA_PATH, STATE, dest_state_intersection_wfeatures, state_geometry):
    # check all preds are within state
    is_contained = gpd.sjoin(dest_state_intersection_wfeatures.to_crs("epsg:4326"), state_geometry, how="inner", predicate="within")
